...
export const useCarToolStore: UseCarToolStore = (initialCars) => {
  ...
  const addCar = useCallback(function addCar(carForm: CarFormData) {
    appendCar(carForm);
    setEditCarId(-1);
  }, [appendCar]);
  ...
  const confirmDeleteCar = useCallback((carId: number) => {
  ...
  const sortCars = useCallback(function sortCars(col: keyof Car) {
    if (col === carsSort.col) {
      setCarsSort({
        col,
  ...
  const cancelConfirmDeleteCar = useCallback(function cancelConfirmDeleteCar() {
    setConfirmDeleteCarMessage({
      message: '',
      carId: -1,
    });
